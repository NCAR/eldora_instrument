...
struct runParams parseOptions(int argc, char** argv, int deviceNumber) {
...
void * dataTask(void* threadArg) {
	...
	DDSPublisher* publisher = pParams->publisher;
...
void showStats(runParams& params, RR314& rr314, unsigned long& droppedPulse,
		unsigned long& droppedTS, int loopCount) {
	...
	std::cout << "Device:" << params.deviceNumber 
	  << "  loop:" << loopCount++ << "\n ";
	std::cout << std::setw(8);
...
int main(int argc, char** argv) {
	...
	try
	{
		RR314 rr314_0(params0.deviceNumber,
				...
				params0.kaiser,
				params0.gaussian,
				params0.simulate // simulation mode?
		);
		...
		RR314 rr314_1(params1.deviceNumber,
				...
				params1.kaiser,
				params1.gaussian,
				params1.simulate // simulation mode?
		);
