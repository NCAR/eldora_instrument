...
DoradeASIB::DoradeASIB(const unsigned char *data, unsigned int datalen, 
    ...
    _antennaHeading = grabFloat(data, 36, isLittleEndian);
    _rollAngle = grabFloat(data, 40, isLittleEndian);
    _pitchAngle = grabFloat(data, 44, isLittleEndian);
    _yawAngle = grabFloat(data, 48, isLittleEndian);
    _antennaScanAngle = grabFloat(data, 52, isLittleEndian);
    _antennaFixedAngle = grabFloat(data, 56, isLittleEndian);
...
DoradeASIB::printTo(std::ostream& os) const
{
    ...
    if (_verbose) {
        ...
        os << "  antenna heading: " << _antennaHeading << std::endl;
        os << "  roll: " << _rollAngle << std::endl;
        os << "  pitch: " << _pitchAngle << std::endl;
        os << "  yaw: " << _yawAngle << std::endl;
        os << "  scan angle: " << _antennaScanAngle << std::endl;
        os << "  fixed angle: " << _antennaFixedAngle << std::endl;
