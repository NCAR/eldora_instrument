...
void fprint_parameter(param_pntr,
                      print_pntr)
    struct parameter_d *param_pntr;FILE *print_pntr;

{
    ...
    fprintf(print_pntr, 
            "PARMID     PARAMETER DESCRIPTOR        : ");
    for (i=0; i<4; i++)
        fprintf(print_pntr, "%c", param_pntr->parameter_des[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr, 
            "PARMDESLEN PARAMETER DESCRIPTOR LENGTH : %ld\n",
            param_pntr->parameter_des_length);
    fprintf(print_pntr, 
            "PARMNAME   PARAMETER NAME              : ");
    for (i = 0; i < 8; i++)
        fprintf(print_pntr, "%c", param_pntr->parameter_name[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr, 
            "PARMDESC   PARAMETER DESCRIPTION       : ");
    for (i = 0; i < 40; i++)
        fprintf(print_pntr, "%c", param_pntr->param_description[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr, 
            "PARMUNITS  PARAMETER UNITS             : ");
    for (i = 0; i < 8; i++)
        fprintf(print_pntr, "%c", param_pntr->param_units[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr,
            "PARMIPP    INTERPULSE PERIODS USED     : 0x%02x\n",
            param_pntr->interpulse_time);
    fprintf(print_pntr,
            "PARMTXFREQ TRANS. FREQUENCIES USED     : 0x%02x\n",
            param_pntr->xmitted_freq);
    fprintf(print_pntr,
            "PARMEFFBW  EFFECTIVE RCVR BW (MHz)     : %f\n",
            param_pntr->recvr_bandwidth);
    fprintf(print_pntr,
            "PARMPULSEW PULSE WIDTH (m)             : %d\n",
            param_pntr->pulse_width);
    fprintf(print_pntr,
            "PARMPOLAR  POLARIZATION                : %d\n",
            param_pntr->polarization);
    fprintf(print_pntr,
            "PARMSAMPLS NUM SAMPLES USED IN ESTIMATE: %d\n",
            param_pntr->num_samples);
    fprintf(print_pntr,
            "PARMBINFRM BINARY FORMAT OF RADAR DATA : %d\n",
            param_pntr->binary_format);
    fprintf(print_pntr, 
            "PARMTHRNAM THRESHOLD FIELD NAME        : ");
    for (i = 0; i < 8; i++)
        fprintf(print_pntr, "%c", param_pntr->threshold_field[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr, 
            "PARMTHRESH THRESHOLD VALUE             : %f\n",
            param_pntr->threshold_value);
    fprintf(print_pntr,
            "PARMSCALE  PARAMETER SCALE             : %f\n",
            param_pntr->parameter_scale);
    fprintf(print_pntr,
            "PARMBIAS   PARAMETER BIAS              : %f\n",
            param_pntr->parameter_bias);
    fprintf(print_pntr,
            "PARMBADFLG BAD DATA FLAG               : %ld\n",
            param_pntr->bad_data);
}
    ...
    if (param_st.interpulse_time != 0)
        ...
        for (i = 0; i < radar[AFT].num_parameter_des; i++) {
            ...
            for (j=0; j<radar[AFT].num_ipps_trans; j++)
                parameter[i][AFT].interpulse_time |= (short)pow(2.0, (double)j);
        ...
        for (i = 0; i < radar[FORE].num_parameter_des; i++) {
            ...
            for (j=0; j<5; j++)
                if (waveform.num_chips[j] > 0)
                    parameter[i][FORE].xmitted_freq |= (short)pow(2.0,
                                                                   (double)j);
        }
