...

{
	...
	printf(" 6.   INPUT THE H PLANE ANGLE\n");
        ...
        gets(input);
        choice = atoi(input);
        if((choice < 0) || (choice >11)) printf("ERROR PLEASE REENTER\n");
...
fprintf(print_pntr,"TIME DELAY (round trip) CHIP TO A TO D CONVERTER (ns):%d\n"
   ...
   fprintf(print_pntr,"HEADER FILE NAME               : %s\n",
...
float num, afloat, afloat1, numsamples;
float max_input_power, max_power_counts, db, proc_gain;
...
if(fradar_st.r_noise_figure != 0)
...
else if(engin_st.forehpa_snum > 0 && engin_st.afthpa_snum > 0)
  {
   ...
   fradar[AFT].r_noise_figure =
     hpa[engin_win.afthpa_snum].r_noise_figure;
   number_to_calculate--;
...
#else
      max_input_power = 30.0 + 
	10 * log10((double)((1.024 * 1.024)/(50.0)));
      /* 30 => gets you to dBm, 1.024^2 => V peak squared,
         50 => load resistance */

      max_power_counts = 10 * 
	log10(pow((double)(2048.0 * 32.0 * 8.0 * engin_win.chipdur),2.0));
...
if(fradar_st.scale_factor[0] != 0)
	...
	radar_st.num_freq_trans)
  {
	...
	db = radar[FORE].radar_const - fradar[FORE].loss_in + fradar[FORE].receiver_gain[i] +
	    fradar[FORE].if_gain[i] + fradar[FORE].conversion_gain +
	      fradar[FORE].x_band_gain;

	/* Now figure the two processing gains due to summing */
	proc_gain = 10.0 * log10((double)radar[FORE].num_freq_trans) +
	  10.0 * log10((double)numsamples);
	...
	fradar[FORE].scale_factor[i] = 
	  pow((double)10.0,(double)((db + proc_gain)/-20.0));
...
/*	printf("dbs = %f  proc_gain = %f  scale_factor = %f\n",
	...
	db = radar[AFT].radar_const  - fradar[AFT].loss_in + fradar[AFT].receiver_gain[i] + 
           fradar[AFT].if_gain[i] + fradar[AFT].conversion_gain +
	     fradar[AFT].x_band_gain;

	/* Now figure the two processing gains due to summing */
	proc_gain = 10.0 * log10((double)radar[AFT].num_freq_trans) +
	  10.0 * log10((double)numsamples);
...
if(fradar_st.dly_tube_antenna != 0)
...
else if( engin_st.avg_freq[0] > 0)
  {
      ...
      afloat = antenna[FORE].tx_waveguide_length / 
	( C * sqrt((double)(1.0 - afloat1 * afloat1)));
      fradar[FORE].dly_tube_antenna = afloat * 1.e9;  /* Convert to ns */

      afloat = engin_win.avg_freq[AFT] * 1.0e9;   /* Convert to hertz */
      afloat1 = C / (2.0 * afloat * WAVEGUIDE_LARGE_DIM);
      afloat = antenna[AFT].tx_waveguide_length /
	( C * sqrt((double)(1.0 - afloat1 * afloat1)));
...
if(fradar_st.dly_modulator_on != 0)
  ...
  {
      fradar[FORE].dly_modulator_on = hpa[engin_win.forehpa_snum].turn_on_time;
      fradar[FORE].dly_modulator_off=hpa[engin_win.forehpa_snum].turn_off_time;
      fradar[AFT].dly_modulator_on = hpa[engin_win.afthpa_snum].turn_on_time;
      fradar[AFT].dly_modulator_off = hpa[engin_win.afthpa_snum].turn_off_time;
...
if(fradar_st.test_pulse_offset != 0)
...
else if(engin_st.schemenum > 0)
  {
	...
	atten[FORE].loss_rawtp_powermeter - 
	...
	atten[AFT].loss_rawtp_powermeter - 
...
if(fradar_st.indep_freq_gate != 0)
...
else if(engin_st.numgates > 0 && engin_st.chipdur > 0)
  {
      ...
      for(i=0; i<4; i++)
	if((integer+i) % 4 == 0) break;
      integer += i;

      /* now make sure it is not too far out in range */
      if(integer > midgate) integer = midgate;
...
if(fradar_st.time_series_gate != 0)
...
else if(engin_st.numgates > 0 && engin_st.chipdur > 0)
  {
	...
	if((integer+i) % 4 == 0) break;
      ...
      /* now make sure it is not too far out in range */
      if(integer > engin_win.numgates) integer = engin_win.numgates - 4;
