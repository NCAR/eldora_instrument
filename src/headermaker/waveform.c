    ...
    struct waveform_d *waveform_pntr;
{
    ...
    for (i = 0; i < 15; i++)
        waveform_pntr->ps_file_name[i] = ' ';

    for (i = 0; i < 6; i++)
        waveform_pntr->num_chips[i] = 0;

    for (i = 0; i < 256; i++)
        waveform_pntr->blank_chip[i] = 0;
    ...
    for (i = 0; i < 6; i++)
        waveform_pntr->chip_offset[i] = -999;

    for (i = 0; i < 6; i++)
        waveform_pntr->chip_width[i] = -999;

    waveform_pntr->ur_pcp = -999.;
    waveform_pntr->uv_pcp = -999.;

    for (i = 0; i < 6; i++)
        waveform_pntr->num_gates[i] = -999;
...
void fprint_waveform(waveform_pntr,
                     print_pntr)
    struct waveform_d *waveform_pntr;FILE *print_pntr;
{
    int i, k;

    fprintf(print_pntr, "***************WAVEFORM DESCRIPTOR*****************\n");
    fprintf(print_pntr, 
            "WAVEID     WAVEFORM IDENTIFIER                       : ");
    for (i=0; i<4; i++)
        fprintf(print_pntr, "%c", waveform_pntr->waveform_des[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr,
            "WAVEDESLEN WAVEFORM DESCRIPTOR LENGTH                : %ld\n",
            waveform_pntr->waveform_des_length);
    fprintf(print_pntr, 
            "WAVEPULFIL PULSING SCHEME FILE NAME                  : ");
    for (i = 0; i < 16; i++)
        fprintf(print_pntr, "%c", waveform_pntr->ps_file_name[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr, 
            "WAVENCHIPS NUMBER OF CHIPS IN A REPEAT               : ");
    for (i = 0; i< 6; i++)
        fprintf(print_pntr, "%d ", waveform_pntr->num_chips[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr, 
            "WAVEBNKRAM BLANKING RAM SEQUENCE                     : ");
    k = 0;
    do {
    ...
    fprintf(print_pntr,
            "WAVEMSREP  NUMBER OF MILLISECONDS PER REPEAT         : %f\n",
            waveform_pntr->repeat_seq);
    fprintf(print_pntr,
            "WAVESEQREP NUMBER OF REPEAT SEQUENCES IN A DWELL TIME: %d\n",
            waveform_pntr->repeat_seq_dwel);
    fprintf(print_pntr,
            "WAVEPCPREP TOTAL NUMBER OF PCP IN A REPEAT SEQUENCE  : %d\n",
            waveform_pntr->total_pcp);
    fprintf(print_pntr, 
            "WAVECHPOFF CHIP OFFSET (60 MHz counts)               : ");
    for (i = 0; i < 6; i++)
        fprintf(print_pntr, "% d", waveform_pntr->chip_offset[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr, 
            "WAVECHPWID CHIP WIDTH (60 MHz counts)                : ");
    for (i = 0; i < 6; i++)
        fprintf(print_pntr, "%d ", waveform_pntr->chip_width[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr,
            "WAVEPCPRNG PCP THAT SET THE UNAMBIGIOUS RANGE        : %f\n",
            waveform_pntr->ur_pcp);
    fprintf(print_pntr,
            "WAVEPCPVEL PCP THAT SET THE UNAMBIGIOUS VELOCITY     : %f\n",
            waveform_pntr->uv_pcp);
    fprintf(print_pntr, 
            "WAVENGATES TOTAL NUMBER OF SAMPLED GATES             : ");
    for (i = 0; i < 5; i++)
        fprintf(print_pntr, " %d", waveform_pntr->num_gates[i]);
    fprintf(print_pntr, "\n");
    fprintf(print_pntr,
            "WAVEGATE1  GATE DISTANCE 1 (60 MHz Counts)           : %d %d\n",
            waveform_pntr->gate_dist1[0],
            waveform_pntr->gate_dist1[1]);
    fprintf(print_pntr,
            "WAVEGATE2  GATE DISTANCE 2 (60 MHz Counts)           : %d %d\n",
            waveform_pntr->gate_dist2[0],
            waveform_pntr->gate_dist2[1]);
    fprintf(print_pntr,
            "WAVEGATE3  GATE DISTANCE 3 (60 MHz Counts)           : %d %d\n",
            waveform_pntr->gate_dist3[0],
            waveform_pntr->gate_dist3[1]);
    fprintf(print_pntr,
            "WAVEGATE4  GATE DISTANCE 4 (60 MHz Counts)           : %d %d\n",
            waveform_pntr->gate_dist4[0],
            waveform_pntr->gate_dist4[1]);
    fprintf(print_pntr,
            "WAVEGATE5  GATE DISTANCE 5 (60 MHz Counts)           : %d %d\n",
            waveform_pntr->gate_dist5[0],
            waveform_pntr->gate_dist5[1]);
}/*end fprint_waveform */
...
#define NS_TO_60MHZ       0.06           /* Multiplier for nano-sec to 60 MHz
    ...
    if (waveform_st.repeat_seq_dwel != 0)
        ...
        afloat = (float)engin_win.numsamples + 0.5
                * pscheme[engin_win.schemenum].samples_in_repeat;
        waveform.repeat_seq_dwel = afloat
                / pscheme[engin_win.schemenum].samples_in_repeat;
        number_to_calculate--;
    ...
    if (waveform_st.chip_offset[0] != 0) /* Have they already been calculated? */
        ...
        for (i = 0; i < 5; i++) {
            if (waveform.num_chips[i] > 0)
                waveform.chip_offset[i] = hpa_on_time * NS_TO_60MHZ
                        + pscheme[engin_win.schemenum].chip_percent[i] * afloat
                                * 0.01 * NS_TO_60MHZ;
            else
        ...
        for (i = 0; i < 5; i++) {
            if (waveform.num_chips[i] > 0)
                waveform.chip_width[i] = (150. * engin_win.chipdur)
                        / METERS_PER_60MHZ;
            else
    ...
    fradar_st.dly_tube_antenna > 0) {
        ...
        fst_gate_60mhz = width/2 + rcvr[FORE].delay_chip_rcvrs * NS_TO_60MHZ
