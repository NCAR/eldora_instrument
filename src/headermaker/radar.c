...
void initialize_radar(radar_pntr,radarnum)
...
if(radar_st.radar_const != 0)
  number_to_calculate--;
else if(radar_st.horz_beam_width > 0 &&
	radar_st.vert_beam_width > 0 && engin_st.chipdur > 0 &&
	radar_st.antenna_gain > 0 && radar_st.peak_power > 0 &&
	fradar_st.loss_out > 0 && fradar_st.loss_rjoint > 0 &&
	engin_st.avg_freq[0] > 0 && fradar_st.loss_in > 0)
  {
      freq_squared = engin_win.avg_freq[FORE] * engin_win.avg_freq[FORE] * 1.0e18;
    ...
    pwrdbm = 30 + 10 * log10((double)(radar[FORE].peak_power * 1000.0))
      ...
      freq_squared = engin_win.avg_freq[AFT] * engin_win.avg_freq[AFT] * 1.0e18;
...
if(radar_st.req_rotat_vel != 0)
  number_to_calculate--;
else if(engin_st.rot_spacing > 0 &&
	waveform_st.repeat_seq > 0 && waveform_st.repeat_seq_dwel > 0)
  {
...
if(radar_st.num_freq_trans != 0)
  ...
  {
      if(engin_win.chipdur == 0.25) delta_freq = 0.012;
      if(engin_win.chipdur == 0.5) delta_freq = 0.010;
      if(engin_win.chipdur == 0.75) delta_freq = 0.009333;
      if(engin_win.chipdur == 1.0) delta_freq = 0.010;
      if(engin_win.chipdur == 1.25) delta_freq = 0.0096;
      if(engin_win.chipdur == 1.5) delta_freq = 0.008666;
      if(engin_win.chipdur == 1.75) delta_freq = 0.009143;
      if(engin_win.chipdur == 2.0) delta_freq = 0.010;
      if(engin_win.chipdur > 2.0) delta_freq = 0.010;
      ...
      for(i=0; i<5; i++)
	{
	    tmp_dbl = (double)rcvr[FORE].cntr_freq + 
	      (double)(freq_mult[i] * delta_freq);
	    freqs[FORE][i] = tmp_dbl;
	    tmp_dbl = (double)rcvr[AFT].cntr_freq + 
	      (double)(freq_mult[i] * delta_freq);
	    freqs[AFT][i] = tmp_dbl;
