...
if(flag)
  {
    ...
    printf(" Pulsing Scheme Name: %s\n",waveform.ps_file_name);
    printf(" Pulse Repetition Frequency #1: %5.0f Hertz\n",prf1);
    printf(" Pulse Repetition Frequency #2: %5.0f Hertz\n",prf2);
    printf(" Number Of Gates Sampled: %5d \n",waveform.num_gates[0]);
    printf(" Chip Width: %7.3f us\n",chip_width);
    printf(" Data Reduction Top Altitude: %5.1f km\n",
	   radar[0].data_red_parm0);  
    printf(" Data Reduction Bottom Altitude: %5.1f km\n",
	   radar[0].data_red_parm1);
    if(fradar[0].indepf_times_flg >= 1)
      printf(" Independent Frequency Data Being Recorded At Gate: %4d\n",
	   fradar[0].indep_freq_gate);
    else
      printf(" Independent Frequency Data Not Being Recorded\n");
    if(fradar[0].indepf_times_flg == 2)
      printf(" Time Series Data Being Recorded At Gate: %4d\n",
	   fradar[0].time_series_gate);
    else
      printf(" Time Series Data Not Being Recorded\n");
    printf(" Data Rate Out Of The Radar Processors: %7.2f Kbytes/sec\n",
	   max_possible_rate);
    printf(" Data Rate Recorded To Tape: %7.2f Kbytes/sec\n",
	   recording_rate);
    printf(" The Duty Cycle Is: %5.2f percent\n",duty_cycle);
    printf(" Rotation Rate:  %7.2f deg/sec\n",radar[0].req_rotat_vel);

    printf("\nSCIENTIFIC PARAMETERS:\n");
    printf(" Total Number Of Data Cells: %4d\n",total_cells);
    printf(" Cell Spacing: %3d m\n",cell[0].spacing[0]);
    printf(" Distance To The First Cell: %3d m\n",cell[0].distToFirst);
    printf(" Rotational Spacing Of Data Beams:  %5.2f deg\n",beam_spacing);
    printf(" Along Track Spacing Of Data Beams:  %7.2f m\n",along_tk_res);
    printf(" Dwell Time For A Data Beam Is: %5.1f ms\n",dwelltime);
    printf(" The Number Of Samples In A Dwell Period: %4d\n",num_samples);
    printf(" The Number Of Independent Samples In A Dwell Period: %6.2f\n",
	   num_indep_samples);
    printf(" The Maximum Sampled Range Is: %7.2f km\n",
	     max_range_gates);
    printf(" The Maximum Possible Range Is: %7.2f km\n",
	     max_range_pcp);
    printf(" The Unambiguous Velocity Range Is: +/-%7.2f m/s\n",
	   unambig_velocity);
