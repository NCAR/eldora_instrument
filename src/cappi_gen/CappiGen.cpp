...
CappiGen::CappiGen(std::string outputFilename, std::string nc_template,
		double angleTol) :
	_outputFilename(outputFilename), _nc_template(nc_template),
			_nc_output(NULL), _recordCount(0), _maxCells(0),
			_angleTol(angleTol), _totalCount(0), _unixTimeVar(NULL),
			_msTimeVar(NULL), _prodTypeVar(NULL) {
...
int CappiGen::openOutput(std::string &output) {
	...
	if (!fs::exists(path) ) {
		...
		if (rc != 0) {
			std::cerr << "couldn't create " << output << " from "
					<< _nc_template << std::endl;
			return (1);
		}
	}
...
void CappiGen::productSlot(std::vector<double> p, int radarId, int prodType,
		qlonglong timetag, StrMapDouble hskpMap) {
	bool ok;
