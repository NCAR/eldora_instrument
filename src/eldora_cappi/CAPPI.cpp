...
#include "CAPPI.h"
double CAPPI::_distanceSpanKm = 100.0;
...
CAPPI::beam::beam(double bxKm,
		double byKm,
		double startAngle,
                double stopAngle,
                int nGates,
                int nVars):
        _bxKm(bxKm), _byKm(byKm),
  _startAngle(startAngle), _stopAngle(stopAngle), _nVars(nVars),
            _nGates(nGates)
{
    std::cerr << "CAPPI::beam::beam called (" << _bxKm << "," << _byKm << ")\n";
    
    moveBeam(_bxKm, _byKm);
...
void CAPPI::beam::moveBeam(double bxKm, double byKm){
  ...
  // compute offset in GL coordinates
    ...
    float cos1 = cos(M_PI*_startAngle/180.0)/_nGates;
    float sin1 = sin(M_PI*_startAngle/180.0)/_nGates;
    float cos2 = cos(M_PI*_stopAngle/180.0)/_nGates;
    float sin2 = sin(M_PI*_stopAngle/180.0)/_nGates;
...
void CAPPI::configure(int nVars,
                    int maxGates,
...
void CAPPI::addBeam(double xPos,
                    double yPos,
                    float startAngle,
                  float stopAngle,
                  int gates,
                  ...
                  std::vector<ColorMap*>& maps)
{
    ...
    if (startAngle <= stopAngle) {
        ...
        b = new beam(xPos, yPos, startAngle, 360.0, _maxGates, _nVars);
        _beams.push_back(b);
        newBeams.push_back(b);
...
void CAPPI::makeDisplayList(beam* b,
                          int v)
{
    //	glGenBuffers();
...
int CAPPI::beamIndex(double startAngle,
                   double stopAngle)
{
...
void CAPPI::makeRingsAndGrids()
{
    ...
    if (ringDelta > 0 && _ringsEnabled) {
        ...
        if (ringLabelIncrement > 0.0) {
            ...
            // creat the labels. Note that we are not creating a label at zero
