...
float dwelltime, along_tk_res;
float prf1, prf2, chip_width, num_indep_samples;
...
num_indep_samples = (1.0 + dwelltime / 5.0) * (float)radar[0].num_freq_trans;
...
fprintf(ptr,"\nGENERAL PARAMETERS:\n");
fprintf(ptr," File Name: %s\n",inname);
volume.proj_name[19] = 0; 
fprintf(ptr," Project Name: %s\n",volume.proj_name);

fprintf(ptr,"\nENGINEERING PARAMETERS:\n");
waveform.ps_file_name[15] = 0;
fprintf(ptr," Pulsing Scheme Name: %s\n",waveform.ps_file_name);
fprintf(ptr," Pulse Repetition Frequency #1: %5.0f Hertz\n",prf1);
fprintf(ptr," Pulse Repetition Frequency #2: %5.0f Hertz\n",prf2);
fprintf(ptr," Number Of Gates Sampled: %5d \n",waveform.num_gates[0]);
fprintf(ptr," Chip Width: %7.3f us\n",chip_width);
fprintf(ptr," Data Reduction Top Altitude: %5.1f km\n",
       radar[0].data_red_parm0);  
fprintf(ptr," Data Reduction Bottom Altitude: %5.1f km\n",
       radar[0].data_red_parm1);
if(fradar[0].indepf_times_flg >= 1)
  fprintf(ptr," Independent Frequency Data Being Recorded At Gate: %4d\n",
	 fradar[0].indep_freq_gate);
else
  fprintf(ptr," Independent Frequency Data Not Being Recorded\n");
if(fradar[0].indepf_times_flg == 3)
  fprintf(ptr," Time Series Data Being Recorded At Gate: %4d\n",
	 fradar[0].time_series_gate);
else
  fprintf(ptr," Time Series Data Not Being Recorded\n");
fprintf(ptr," Data Rate Out Of The Radar Processors: %7.2f Kbytes/sec\n",
       max_possible_rate);
fprintf(ptr," Data Rate Recorded To Tape: %7.2f Kbytes/sec\n",
       recording_rate);
fprintf(ptr," The Duty Cycle Is: %5.2f percent\n",duty_cycle);
fprintf(ptr," Rotation Rate:  %7.2f deg/sec\n",radar[0].req_rotat_vel);

fprintf(ptr,"\nSCIENTIFIC PARAMETERS:\n");
fprintf(ptr," Total Number Of Data Cells: %4d\n",total_cells);
fprintf(ptr," Cell Spacing: %3d m\n",cell[0].spacing[0]);
fprintf(ptr," Distance To The First Cell: %3d m\n",cell[0].distToFirst);
fprintf(ptr," Rotational Spacing Of Data Beams:  %5.2f deg\n",beam_spacing);
fprintf(ptr," Along Track Spacing Of Data Beams:  %7.2f m\n",along_tk_res);
fprintf(ptr," Dwell Time For A Data Beam Is: %5.1f ms\n",dwelltime);
fprintf(ptr," The Number Of Samples In A Dwell Period: %4d\n",num_samples);
fprintf(ptr," The Number Of Independent Samples In A Dwell Period: %6.2f\n",
       num_indep_samples);
fprintf(ptr," The Maximum Sampled Range Is: %7.2f km\n",
       max_range_gates);
fprintf(ptr," The Maximum Possible Range Is: %7.2f km\n",
       max_range_pcp);
fprintf(ptr," The Unambiguous Velocity Range Is: +/-%7.2f m/s\n\n\n",
       unambig_velocity);
