...
float diff, tdiff;
  ...
  frequency = fore_freqs[tp_freq_count] + (double)fore_freq_offset[tp_freq_offset_count];
  ...
  do
  {
    ...
    timeout++;
  } while((test = ecbSetDDS(ecbaddr,unitnum,B,T,nr) != 0) && timeout < 30);
  ...
  do
  {
    if(timeout > 0) taskDelay(1);
    timeout++;
  } while((test = ecbSetAtten(ecbaddr, foraft, attenuation) != 0) && timeout < 30);  

  // Now re-program the aft testpulse with a new value 

  aft_vmehndshk->tpulse.freq_num[0] = tp_freq_count + 1;
  frequency = aft_freqs[tp_freq_count] + (double)aft_freq_offset[tp_freq_offset_count];
