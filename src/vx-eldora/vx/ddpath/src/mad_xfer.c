 ...
 * ----------------
 * $Log$
 * Revision 1.1  1991/06/24  15:50:56  thor
 * Initial revision
 *
 *
 * description:  This program copies what the radar processor must do to
 *               the Multi Crate Pipeline (MCPL) but stops when all MAD
...
int mad_xfer(int cells, int params, int binary)
{
#define LOG_HEADER_LEN 104  /* length in Bytes of radar logical record header*/
    ...
    long *tpb_to_proc_pntr,*gmad_to_proc_pntr,*receive_gmad_pntr,*tmp_pntr;
    ...
    int number_of_mads[3]; /* Number of mads in each processor 0=> no */
			   /* processor */
...
#define MAX_MEM      6
    ...
    long *gmad_pntr;
    /* Must be absolutely certain this lines up on a 4 byte boundary. */
    /* asm(".align4");*/
    /* short temp_align; */
    struct MAD gmad;
    ...
    init.link_attribute = 0x3933;     /* Normal VMEBus Addressing, VME */
				      /* link after trigger, address */
				      /* modifier $39, disable BLT, 32 */
				      /* Xfers/burst, disable */
				      /* pipelineing, link to VME */
    ...
    /* If MCPL has not been initialized since power on status bit 0 */
    /* will be set */
    ...
    /* if this is not the first time the program has run, we need to */
    /* send the hold command, but the card will respond by a "hold */
    /* acknowledged" status in the mailbox */
    ...
    *start_reg = 0x0001;
    ...
    if(mailbox == 0x1fff)
	  ...
	  printf("Init TPB transfer unsuccessful mailbox: %x status: %x\n",
		 mailbox,status);
	  return(MCPL_BADTRANS);
      }
    
    /*************************************************************************/
    /* Setup Generic parts of TPB's to send a MAD structures to the */
    /* processors  */
    /*************************************************************************/
    
    /* The tpb_to_proc TPB sends the receive_gmad_tpb to a processor's */
    /* dispatch table entry one.  All of the general purpose parmeters */
    /* in the TPB will be set now and processor specific ones will be */
    /* set later */
    ...
    tpb_to_proc.completion_status = 0x0002;  /* Disable interrupt */
					     /* Mailbox  A24/D16*/
    tpb_to_proc.nack_count = 0;              /* Do not retry */
    tpb_to_proc.link_address = (long)gmad_to_proc_pntr;
    /* link to gmad_to_proc TPB structure */
    tpb_to_proc.link_attribute = 0x3963; /* Normal VMEBus */
					 /* Addressing,VME link  after
					    trigger, address modifier
					    $39, d isable
    ...
    /* The gmad_to_proc TPB sends the a MAD to an MCPL node as determined by 
       its dispatch table entry one.  All of the general purpose
       parmeters in this TPB will be set now and processor specific
       ones will be set later */
    ...
    gmad_to_proc.sink_start_address = 0;      /* Going to an mcpl node so
						 no address is necessary */
    gmad_to_proc.source_attribute = 0x3963; /* Normal VMEBus */
					    /* Addressing,  address
					       modifier $39, disable
					       pipelining,
					       disable BLT, 256
    ...
    /* The receive_gmad tpb loads data off of the MCPL and into VMEBus memory
       this TPB is loaded in dispatch table entry one, completion of
       transfer is not signaled */
    
    receive_gmad.source_start_address = 0; /* Data to be retrieved from mcpl */
    receive_gmad.sink_start_address = 0x0000000; /* Generic Start */
						 /* Address of  the
						    MAD Memory */
    ...
    receive_gmad.link_address = 0;         /* Linking is not performed */
					   /* by target */ 
    receive_gmad.link_attribute = 0;       /* Linking is not performed */
					   /* by target */ 
    ...
    logical_length = LOG_HEADER_LEN + (cells * params * binary);
    
    printf("calculated logical record length: %d\n",logical_length);
    ...
    for (proc=0; proc<3; proc++)
	  ...
	  for (mad=0; mad<number_of_mads[proc]; mad++)
		...
		if((madinfo[proc][mad][NUM_AREAS] = 
		    (madinfo[proc][mad][MAX_MEM]-0x1800)/
		    madinfo[proc][mad][SIZE_AREAS])>1000)
		  madinfo[proc][mad][NUM_AREAS] = 1000;
		printf("Proc: %d MAD: %d area size: %d number of areas: %d\n",
		       proc,mad,madinfo[proc][mad][SIZE_AREAS],
		       madinfo[proc][mad][NUM_AREAS]);
		
		/* fill the general MAD structure with the proper */
		/* information */ 
		...
		for(i=0; i<madinfo[proc][mad][NUM_AREAS]; i++)
		  {
		      gmad.mailboxes[i] = 0;
		      gmad.addresses[i] = madinfo[proc][mad][MAD_BASE]
			+ 0x1800 + i * madinfo[proc][mad][SIZE_AREAS];
		  }
		
		/* Fill Generic holes in TPB's to send this MAD */
		/* structure to the 
		   proper processor */
		tpb_to_proc.mcpl_control_word = 
		  (tpb_to_proc.mcpl_control_word & 0x07FF)
		    | (proc_nodes[proc]<<11);
		gmad_to_proc.mcpl_control_word= 
		  (gmad_to_proc.mcpl_control_word & 0x07FF)
		    | (proc_nodes[proc]<<11);
		receive_gmad.sink_start_address = madinfo[proc][mad][MAD_BASE];
		
		printf("num_before_repeat: %d\n",gmad.num_before_repeat);
		printf("source transfer length %d\n",gmad_to_proc.transfer_length);
		printf("target transfer length %d\n",receive_gmad.transfer_length);
		printf("source start address %lx\n",gmad_to_proc.source_start_address);
		tmp_pntr = (long *)gmad_to_proc.source_start_address;
		for(i=0; i<10; i++)
		  printf("address: %lx  value: %lx\n",(tmp_pntr+i),*(tmp_pntr+i));
