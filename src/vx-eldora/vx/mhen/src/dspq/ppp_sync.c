 ...
 * description:
 *
 *  Tests for synchronization of individual DSP32C's on Pulse Pair Processor.
 *  arg1 = frequency; 1,2, or 3 of PPP
 *  arg2 = board  
 *        
 */
...
unsigned long bd,frq;
{
    ...
    for(i=0;i<4;i++)
	  ...
	  *(pio + PCRL) = AUTO + DMA + ENI + REGMAP + RUN;    /* set up DMA and AUTO */
	  ...
	  parm = (unsigned int )PPP_SYNC_FLAG;
	  ...
	  *(pio + PARH) = (0xff00 & parm) >> 8;
	  ...
	  if(status != 0)
	    {
		flag |= 0x1;
		printf("PPP OUT OF SYNC ERROR in Board %x, Processor %d \n",bd,i);
		/* Turn off LED on PPP */
		...
		*led = 0x0;
		...
		pio = (unsigned char *)(bd + (DSPSEL + i * DSPOFF));
		if(*(pio + PCRL) != 0)scr = *(pio + PDRH);
		parm = (unsigned int )PPP_EOF_RCVD;
		*(pio + PARE) = (0xff0000 & parm) >> 16;
		*(pio + PARL) = 0xff & parm;
		*(pio + PARH) = (0xff00 & parm) >> 8;
		temp = (unsigned char *)(pio + PDRL);
