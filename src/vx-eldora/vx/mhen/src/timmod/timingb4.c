 ...
 * ----------------
 * $Log$
 * Revision 1.1  1992/09/11  20:44:08  shawn
 * Initial revision
 *
 *
 ...
 *
  Purpose:       Program ELDORA timing module rev b to generate a single prf
		 on F0,F1,F2 and F3, and a pre-delayed preknock.
  ...
  Date Written:  09/11/92  (copied timingb3.c)
 *        
 */
...
void timingb4(prf,numgates,gatespace,duty)
     ...
     
{
  ...
  unsigned short *timgate0,*timgate1,*timgate2,*timgate3;
  unsigned short *timpcpn,*timblank,*timchip,*timoff;
  unsigned short pcpn,chipcounts,gatecounts;
  unsigned short f0start,f0end,f1start,f1end,f2start,f2end,f3start,f3end;
...
/* Pointers to Collator */
  ...
  printf("Hello, this is timingb4.  Usage:\n");
  printf("timingb4 prf[Hz],numgates,gatespace[m],duty[%%],\n\n");
  printf("values actually passed to timingb4==> %d,%d,%d,%f\n",prf,numgates,gatespace,duty);
  ...
  if (prf < 250)
    {
      printf("timingb4: prf must be greater than 250 Hz.\n");
      printf("timingb4: aborting without programming timing module...\n\n");
      return;
    }
  if (prf > 20000)
    {
      printf("timingb4: requested prf is greater than 20000.\n");
      printf("timingb4: aborting without programming timing module...\n\n");
      return;
    }
  ...
  if (numgates > 1024)
    {
      printf("timingb4: requested number of gates is greater than 1024.\n");
      printf("timingb4: aborting without programming timing module...\n\n");
      return;
    }
  ...
  if ((gatespace < 30) | (gatespace > 1200))
    {
      printf("timingb4: gate spacing must be between 30 and 1200 meters.\n");
      printf("timingb4: requested spacing was %d meters.\n",gatespace);
      printf("timingb4: aborting without programming timing module...\n\n");
      return;
    }
  ...
  if (gatetime > 0.98*prt)
    {
      printf("timingb4: gates fill more than 98%% of a prt.\n");
      printf("timingb4: time for all gates = %f usec\n",gatetime*1000000);
      printf("timingb4: time in one prf    = %f usec\n",prt*1000000);
      printf("timingb4: aborting without programming timing module...\n\n");
      return;
    }
  ...
  if (duty > 1.0)
    {
      printf("timingb4: duty cycle must not be more than 1%%.\n");
      printf("timingb4: requested duty cycle = %f%%.\n",duty);
      printf("timingb4: aborting without programming timing module...\n\n");
      return;
    } 
  ...
  if (pulsewidth > 0.000010)
    {
      printf("timingb4: requested pulsewidth is more than 10 usec.\n");
      printf("timingb4: pulsewidth = %f usec\n",pulsewidth*1000000);
      printf("timingb4: aborting without programming timing module...\n\n");
      return;
    }
  ...
  *timblank = (unsigned short) 0x040f;

  /* ======================DO CHIPS======================= */
  /* quantize chipwidth (for one of the three chips) */
  chipcounts = realduty * 15000000.0 / prf;
  /* compute start and end of pre-knock, f0, f1, f2 and f3 */
  pkstart = 1;
  ...
  if (maxindex > 4090)
    {
      printf("timingb4: index computation error.  CHIP RAM bounds exceeded.\n");
      printf("timingb4: aborting without fully programming timing module.\n\n");
      return;
    }
  ...
  *timchip = 0x8000;  /* Load initial, special value */
  ...
  for (chipindex = 1; chipindex<maxindex; chipindex++)
	...
	{
	  printf("timingb4: CHIP RAM index computation error.\n");
	  printf("timingb4: aborting without fully programming timing module.\n\n");
	  return;
	}
    }
  ...
  for (gateindex=1; gateindex<numgates; gateindex++)
	...
	*timgate3 = gatevalue;
	...
	if(timgate0 > (unsigned short *) TIMBASE+TIMGATE0+4090)
	  {
	      printf("timingb4: GATE RAM index computation error.\n");
	      printf("timingb4: aborting without fully programming timing module.\n\n");
	      return;
	  }
	timgate1++;
  ...
  *timgate3 = 0x0000;
  ...
  printf("values actually setup by timingb4:\n");
  printf("  prf       = %f\n  numgates  = %d\n",15000000.0/(pcpn+1),numgates);
  printf("  gatespace = %f\n  duty      = %f%%\n",gatecounts*2.5,duty);
	    
printf("timingb4: finished.\n");
