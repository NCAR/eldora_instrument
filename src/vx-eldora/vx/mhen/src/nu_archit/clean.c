...
dac()
{
  ...
  int            *samps, data_samps, buff_cnt, prf, gates, num1, gate_sp, shortcnt,
    ...
    pick_gate, f_flag, taskid, dc_stat, temp_stat[5], pcpcnt[10], eth_stat, hdrsz;
  ...
  *pio_acc = 0x80;                 /* Set bit A7 to one */
  ...
  /* Initialize Noise Subtraction Variables */
  ...
  for(;;)
      ...
      if(sem_status == OK)
	{
	  ...
	  dc_removal = 1; /* enable RP7 PPP to remove DC on first dwell */
	  ...
	  if(!(weather && selection))
		...
		{
		  Cloudy = 0;
		  /* Initialize noise correction Fudge Factor for fore and aft */
	  ...
	  */
	  ...
	  timon();
#endif
	  currStatus->rp7 |= DC_BUSY;   /* tell host we are doing dc_removal */
	  ...
	  while(!(stop || reboot))
	      ...
	      if (sem_status == OK)
		{
		  ...
		  if(vme2_pntr->tpulse_flg == 1)
		    {
		      testpulseb(prf,1,vme2_pntr->tpulse.freq_num[0]-1,
				 vme2_pntr->tpulse_dist,
				 vme2_pntr->tpulse_width);
		  ...
		  while (msgQReceive(VMEQue,(char *)&hbox,2,0) != ERROR)
		    {
		      curr_ray_add[p][r] =
			(long)(VMEMEM_BASE + STD_BASE + DATA_RAY_BASE
			       + (hbox * DATA_RAY_OFFSET));
			...
			Beamcount += 1;
		      ...
		      eth29SendDgram(dataSendToIP, 3102,
				     ...
				     logical_length); /* send data out eth29 */
	  ...
	  dc_flag = 0;
	  dc_removal = 0;
	  currStatus->rp7 &= DC_STAT_MASK;  /* clear DC_STAT */
