...
// Description : Displays combination of different characters on LCD screen.  Lines are compressed.
//					 - optimizing delay times
...
int main(int argc, char **argv) {
	...
	while(!feof(stdin)) {
		...
		//printf("Length: %d\n", len);
		writechars(buf2);
		usleep(1000000);	// added this delay and removed delays in writechar
	}
...
void writechars(unsigned char *dat) {
	...
	do {
		...
		*phdr = ctrl;
		...
		// step 2
		//i = SETUP;
		//COUNTDOWN(i);		// could remove these delays due to usleep in main function
