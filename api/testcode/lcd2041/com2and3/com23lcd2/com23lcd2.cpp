...
// The loop is continuous but can be aborted by the Ctrl+\ command
//============================================================================
...
char dtplace[4]; 	// after decimal, tenth place
char dhplace[4]; 	// hundredth place
...
int main(int argc, char **argv)
{
	...
	// abort signals
	signal(SIGQUIT,cleanup);     // Normally Ctrl+'\'
...
void pc104init(void)
{
	...
	*dio24_r3 = 0x00; // disable all interrupts
...
void lcdsetup(void)
{
	...
	// After decimal point, tenth place
	dtplace[0] = 254;
	dtplace[1] = 35;
...
{
	// calculating random value between the range of 0 and val
	return ((double)(0 + (rand()%(1+(val-0)))));
}

void switchthing(int ff, char *place, int num)
{
	// writing the number to the LCD screen
	switch(num)
	{
		case 0:
			place[3] = 0;
