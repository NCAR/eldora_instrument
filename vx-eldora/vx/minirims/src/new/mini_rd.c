...
short *read;
unsigned short temp_sel[6], rsin, rcos;
int delta;
char *units;
double fsin, fcos, rdc, ftan;
float scale;
int i, j, k, sem_status, temp_cnt, t_cnt;
...
adc_busy=(volatile unsigned short *)(MIN_BASE+ADC_STAT);
		...
		if(sem_status == OK)
		  {
			...
			*sel_sin = 0x0;
			...
			/* Scale by max counts w/headroom */
