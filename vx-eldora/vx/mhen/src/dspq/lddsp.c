...
char *tfile;
{
  ...
  i = 28;
  mnt_pnt = malloc(i);
  mnt_pnt = "odin:/ELDORA/vx/mhen/tadpole/bin";
...
/*      for(dl=0;dl<1024;dl++)dd = dl;              A slight delay */
	...
	/* If a xfer happened, compute the checksum */
        *(pio + PARH) = dspa.pa[1];
        ...
        if(chksumi != chksumo)
        {
           printf("Checksum error on load:\n");
           printf("     Load file checksum = %x\n",chksumo);
           printf("     Readback  checksum = %x\n",chksumi);
        }
        else printf("Load successful: checksum = %x\n",chksumi);
      }
      strcpy(ptr,test);
      flg = 0;
      chksumo = 0;
...
/* Note: bytes are in 680X0 order */
  ...
  else if(!strcmp(test,".ram2:"))   adr = 0x000eff00;
  else adr = 0xffffffff;
  return(adr);
}
