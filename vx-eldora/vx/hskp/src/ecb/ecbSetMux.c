...
/*****************************************************************************/
/*	ecbSetMux()                                                          */
/*		Write "Set Trigger Mux Value" command to ECB                 */
/*              MASTER IN FIFO                                               */
...
unsigned char ecbSetMux(unsigned char ecbadr,unsigned char xmittp,unsigned char muxval)
{
    ...
    if (semTake(ecb_cmd_not_pending,1) == ERROR)
      {
	  printf("ecbSetMux: command still pending...");
    ...
    if (ecbadr!=ECBATTEN)
      {
	  printf("ecbSetMux: ecbadr passed [%d] is not the ",ecbadr);
	  printf("ECB bus address\n");
	  printf("ecbSetMux: for the ATTENUATOR chassis...");
	  printf("Returning without issuing command.\n");
	  printf("ecbSetMux: Re-Giving ecb_cmd_not_pending Semaphore.\n");
	  if(semGive(ecb_cmd_not_pending) == ERROR)
	    {
		printf("ecbSetMux: semGive(ecb_cmd_not_pending) ");
		printf("returned ERROR,\n");
		printf("ecbSetMux: indicating an invalid semaphore ID.\n");
		printf("ecbSetMux: Returning without Give-ing semaphore.\n");
		return(4);
	    }
	  return(5);
      }
    ...
    if (xmittp != 0 && xmittp != 1)
      {
	  printf("ecbSetMux: xmittp passed [%d] is not ",xmittp);
	  printf("a valid value (0 or 1).\n");
	  printf("ecbSetMux: Returning without issuing command.\n");
	  printf("ecbSetMux: Re-Giving ecb_cmd_not_pending Semaphore.\n");
	  if(semGive(ecb_cmd_not_pending) == ERROR)
	    {
		printf("ecbSetMux: semGive(ecb_cmd_not_pending) ");
    ...
    /* Check for valid muxval */
    if ((muxval<ECBATLOWMUX) || (muxval>ECBATHIGHMUX) || (muxval==ECBATBADMUX))
      {
	  printf("ecbSetMux: muxval value passed [%u] is out ",muxval);
	  printf("of range (0..6 & 8..14).\n");
	  printf("ecbSetMux: Returning without issuing command.\n");
	  printf("ecbSetMux: Re-Giving ecb_cmd_not_pending Semaphore.\n");
	  if(semGive(ecb_cmd_not_pending) == ERROR)
	    {
    ...
    if ( (*vmestat & 0x01) )
      {
	  printf("ecbSetMux: command not pending, but ");
    ...
    *infifo = (unsigned char) 2;  /* write numbytes into IN FIFO */
    ...
    if(xmittp==1)       /* check for xmit */
      *infifo = 0x12;   /* write command ID Code into IN FIFO */
    else if (!xmittp)   /* check for testpulse */
      *infifo = 0x13;   /* write command ID Code into IN FIFO */

    *infifo = muxval;   /* write muxval into IN FIFO */
