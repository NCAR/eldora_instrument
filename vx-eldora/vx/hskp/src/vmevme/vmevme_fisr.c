...
float position, fore_angle, elapsed_time, temp_position;
float degrees_moved;
...
if(fore_vmehndshk->radar_hndshk[fore_current_index] == 0)
  fore_vmehndshk->radar_hndshk[fore_current_index] = 1;

/* Create a pointer to a data ray structure at the current radar data
   ray location */

fore_current_offset += RADAR_SIZE_INCR;
fore_current_index++;
if(fore_current_index >= NUM_RADAR_HNDSHK)
  {
      fore_current_offset = FIRST_RADAR_OFFSET;
      fore_current_index = 0;
  }

fore_ray_pntr = (struct DATARAY *)(fore_current_offset + STANDARD_BASE +
                            FORE_STAND_START); 
...
last_fore_msecs_today = msecs_today;
last_fore_day = day;
...
last_fore_position = temp_position;

/* Now we need to update the sweep number for
   each of the radars */

if(degrees_moved = fore_angle - last_fore_angle < 0.)
  degrees_moved = -1. * degrees_moved;
last_fore_angle = fore_angle;

if(degrees_moved > 10.) fore_sweep_num++;
