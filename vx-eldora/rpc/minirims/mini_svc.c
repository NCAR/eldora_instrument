...
static void
minirimscontrol_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
...
main()
{
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		if (!svc_reg(transp, MinirimsControl, MinirimsCtrlVers, minirimscontrol_1, 0)) {
			_msgout("unable to register (MinirimsControl, MinirimsCtrlVers).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (SIG_PF) closedown);
	...
	if (!svc_create(minirimscontrol_1, MinirimsControl, MinirimsCtrlVers, "netpath")) {
		_msgout("unable to create (MinirimsControl, MinirimsCtrlVers) for netpath.");
		exit(1);
	}
