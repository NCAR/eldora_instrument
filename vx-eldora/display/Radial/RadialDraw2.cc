...
void Radial::Draw2(FAST RadialData &data)
{
    ...
    FAST unsigned char *colors = (unsigned char *)&data.colors[0];

    FAST int fg = firstGate;

    FAST int c1 = cos1 * fg + 511;
    FAST int c2 = cos2 * fg + 511;
    FAST int s1 = sin1 * fg + 511;
    FAST int s2 = sin2 * fg + 511;

    short work[5110];
    FAST short *op = work;

    FAST unsigned char oldc = 0xff;
    
    for (FAST int i = fg; i < j; i++, colors++, 
	 c1 += cos1, c2 += cos2, s1 += sin1, s2 += sin2)
      {
	  FAST int x1 = c1 >> 16;
	  FAST int x2 = c2 >> 16;
    ...
    i = (int)op - (int)&work[0];
    i /= 4;
    agc->qpdmCmd((long *)work,i);

//     colors = (unsigned char *)&data.colors[j];
// 
//     c1 = cos1 * fg + PLOT_WIDTH + 511;
//     c2 = cos2 * fg + PLOT_WIDTH + 511;
//     s1 = sin1 * fg + 511;
//     s2 = sin2 * fg + 511;
// 
//     op = work;
