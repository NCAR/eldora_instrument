...
arcfft(complex c[], int n)
   {
   int          i,l,j,p,q,r,m,n1,km,k,k1,k2,t,u,v,w;
   double       scale;
   complex      swap,eta;
...
hann(complex a[], int n)
   ...
   for(i=0; i<n; i++)
      {
      a[i].x *= (scale = 1.0 - zeta[i].x);
      a[i].y *= scale;
      }
   }

hamm(complex a[], int n)
   {
   int          i;
   double       scale;
   ...
   for(i=0; i<n; i++)
      {
      a[i].x *= (scale = 1.08 - 0.92 * zeta[i].x);
      a[i].y *= scale;
      }
   }
